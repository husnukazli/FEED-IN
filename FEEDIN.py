...
is_read_only = query_params.get("view", "") == "true"

# --- ÖZEL CSS (Ağaç Simetrisi ve PDF Çıktısı İçin) ---
st.markdown("""
<style>
/* Fikstür Kart Tasarımı */
.match-card { border: 1px solid #1f77b4; border-radius: 6px; padding: 6px; margin-bottom: 5px; background-color: #f8f9fa; box-shadow: 2px 2px 5px rgba(0,0,0,0.05); }
...
/* PDF ve Yazdırma (Print) Ayarları - CTRL + P ile çalışır */
@media print {
    header, footer, .stExpander, .no-print, [data-testid="stSidebar"], .stRadio, .stSelectbox { display: none !important; }
    ...
    * { -webkit-print-color-adjust: exact !important; color-adjust: exact !important; }
    .match-card { page-break-inside: avoid; border: 1px solid #000; background-color: #eee !important; margin-bottom: 2px !important; }
...
# --- VERİ TABANI YÖNETİMİ (Kalıcı Kayıt İçin) ---
...
if not is_read_only:
    ...
    c_info1.info("🔗 **Paylaşım Linki:** URL'nin sonuna `?view=true` ekleyerek Salt Okunur paylaşabilirsiniz.")
    c_info2.success("🖨️ **PDF/Yazdır:** Çıktı almak için klavyenizden **Ctrl + P** (Mac için Cmd + P) tuşlarına basın.")
else:
    st.warning("👁️ **İzleyici Modu:** Bu sayfa salt okunurdur. Skorlara müdahale edilemez.")

# --- KATEGORİ SEÇİMİ ---
if not is_read_only:
    active_cat = st.radio("Turnuva Kategorisi:", ["Erkekler", "Kadınlar"], horizontal=True, key="cat_sel")
else:
    active_cat = st.selectbox("Turnuva Kategorisi (İzleyici Modu):", ["Erkekler", "Kadınlar"], key="cat_sel_ro")
...
def match_card(m_id, p1, p2, label):
    ...
    if is_read_only:
        if current_winner != "-":
            st.markdown(f"<div style='text-align:center; font-size:12px; color:green;'><b>K:</b> {current_winner} <br> {current_score}</div>", unsafe_allow_html=True)
    ...
    if p1 and p2:
        ...
        if score != current_score or winner != current_winner:
            ...
            if winner != "-":
                ...
                del cat_data['res'][m_id] # Kazanan iptal edildiyse temizle
            save_data()
...
# ==========================================
# 1. ANA TABLO (Eksiksiz Yatay Simetri)
# ==========================================
with tab_ana:
    if not is_read_only:
        with st.expander("👥 Oyuncu Listesi (Esame Sırasına Göre)", expanded=False):
    ...
    m_r1, m_qf, m_sf = {}, {}, {}
    axis_gap = 70 # Yatay simetri eksenindeki orta boşluk

    with c1: 
        st.markdown("<h5 style='text-align: center;'>Son 16</h5>", unsafe_allow_html=True)
        for i in range(4): # Eksen Üstü 4 Maç
            m_r1[i] = match_card(f"MR1_{i}", p[i*2], p[i*2+1], f"R1-M{i+1}")
        spacer(axis_gap) # SİMETRİ MERKEZİ EKSENİ
        for i in range(4, 8): # Eksen Altı 4 Maç
            m_r1[i] = match_card(f"MR1_{i}", p[i*2], p[i*2+1], f"R1-M{i+1}")
    ...
    with c2: 
        ...
        spacer(axis_gap + 75) # SİMETRİ MERKEZİ EKSENİ GEÇİŞİ
...
with tab_teselli:
    ...
    with tc1: 
        ...
        spacer(axis_gap + 150) # Simetri Ekseni
        for i in range(2, 4): 
            c_r1[i] = match_card(f"CR1_{i}", m_r1[i*2][1], m_r1[i*2+1][1], f"T-R1 M{i+1}")
    ...
    with tc3: 
        ...
        spacer(axis_gap + 250) # Simetri Ekseni
    ...
    with tc4: 
        ...
        spacer(axis_gap + 280) # Simetri Ekseni
...
with tab_program:
    ...
    if not is_read_only:
        ...
        tablo_filtresi = "Tümü"
        st.info("📅 Güncel Maç Programı Listesi")
            ...
            if secilen_gun in ["Tüm Günler", day_name]:
                ...
                for m_id, label in filtered_matches:
                    ...
                    if is_read_only:
                        ...
                        new_saat = c4.text_input("Saat", value=data.get("saat", ""), key=f"time_{cat}_{m_id}", label_visibility="collapsed", placeholder="10:00")
