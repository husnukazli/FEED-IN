...
st.title("🎾 Turnuva Fikstür Yönetimi")
...
def draw_match(m_id, p1, p2, label):
    ...
    st.markdown(f'<div class="match-box">{p1 if p1 else "..."}<br>{p2 if p2 else "..."}</div>', unsafe_allow_html=True)
...
# --- TABS ---
tab_ana, tab_teselli, tab_siralama, tab_program = st.tabs(["🏆 Ana Tablo", "🔄 Teselli", "📊 Sıralama", "📅 Program"])
...
with tab_teselli:
    ...
    with c2:
        ...
        c_r2 = {i: draw_match(f"CR2_{i}", c_r1[i][0], qf_losers_reversed[i], f"T-R2 M{i+1}") for i in range(4)}
        spacer(30)
    with c3:
        spacer(140)
...
with tab_program:
    ...
    st.write("Veri yönetimi için diğer sekmeleri kullanabilirsiniz.")
    def edit_day_schedule(matches, day_name):
        st.markdown(f"#### {day_name}")
        cols = st.columns([1.5, 2, 2, 1, 1, 1])
        cols[0].markdown("**Maç**"); cols[1].markdown("**Oyuncu 1**"); cols[2].markdown("**Oyuncu 2**"); cols[3].markdown("**Saat**"); cols[4].markdown("**Kort**"); cols[5].markdown("**Skor**")
        for m_id, label in matches:
            ...
            c = st.columns([1.5, 2, 2, 1, 1, 1])
            c[0].write(label); c[1].write(p1); c[2].write(p2)
            new_saat = c[3].text_input("Saat", value=data.get("saat", ""), key=f"time_{active_cat}_{m_id}", label_visibility="collapsed")
            new_kort = c[4].text_input("Kort", value=data.get("kort", ""), key=f"court_{active_cat}_{m_id}", label_visibility="collapsed")
            new_skor = c[5].text_input("Skor", value=data.get("skor", ""), key=f"prog_score_{active_cat}_{m_id}", label_visibility="collapsed")
