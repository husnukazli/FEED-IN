...
def match_card(m_id, p1, p2, label):
    ...
    name1 = p1 if p1 else "..."
    name2 = p2 if p2 else "..."
    st.markdown(f"""<div style="border: 1px solid #444; padding: 3px; border-radius: 4px; margin-bottom: 2px; font-size: 10px; background-color: #f0f0f0; width: 120px;">{name1}<br>{name2}</div>""", unsafe_allow_html=True)
...
with tab_ana:
    ...
    with c3: # 2 Maç (Offset: 240px, Gap: 575px)
        spacer(240)
        m_sf = {}
        for i in range(2):
    ...
    with c4: # Final (Offset: 535px)
        spacer(535)
        match_card("FINAL_MAIN", m_sf[0][0], m_sf[1][0], "FİNAL")

# --- TESELLI (SİMETRİK PİRAMİT) ---
with tab_teselli:
    st.subheader("🔄 Teselli Fikstürü")
    c_col1, c_col2, c_col3, c_col4, c_col5 = st.columns([1, 1, 1, 1, 1])
...
with tab_program:
    ...
    secilen_gun = st.radio("🗓️ Günü Seçin:", ["Tüm Günler", "1. GÜN", "2. GÜN", "3. GÜN"], horizontal=True)
    st.divider()
