...
def match_card(m_id, p1, p2, label):
    ...
    # Tasarım daha kompakt hale getirildi
...
# --- ANA TABLO (SİMETRİK PİRAMİT YAPISI) ---
with tab_ana:
    ...
    # Her maç bloğunun yüksekliği (card + padding) ~140px kabul edilmiştir.
    # Bu değerlere göre simetriyi kuruyoruz.
    
    with c1: # 8 Maç (R1)
        m_r1 = {i: match_card(f"MR1_{i}", p[i*2], p[i*2+1], f"R1-M{i+1}") for i in range(8)}
    
    with c2: # 4 Maç (ÇF) - İlk maçı R1'in ortasına itmek için 70px padding
        spacer(70) 
        m_qf = {}
        for i in range(4):
            m_qf[i] = match_card(f"MQF_{i}", m_r1[i*2][0], m_r1[i*2+1][0], f"ÇF-M{i+1}")
            spacer(210) # Maçlar arası boşluk
            
    with c3: # 2 Maç (YF) - İlk maçı ÇF'lerin ortasına itmek için 210px padding
        spacer(210)
        m_sf = {}
        for i in range(2):
            m_sf[i] = match_card(f"MSF_{i}", m_qf[i*2][0], m_qf[i*2+1][0], f"YF-M{i+1}")
            spacer(490) # Maçlar arası boşluk
            
    with c4: # 1 Maç (Final)
        spacer(490)
        res_main = match_card("FINAL_MAIN", m_sf[0][0], m_sf[1][0], "FİNAL")

# ... Diğer Tablar (Teselli, Sıralama, Program) olduğu gibi kalmıştır ...
with tab_teselli:
    st.info("Teselli tablosu mantığı ana tablodan gelen mağlup oyunculara göre otomatik çalışır.")
with tab_siralama:
    st.header("Sıralama")
with tab_program:
