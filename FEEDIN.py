...
with tab_ana:
    ...
    # ÇF Maçları
    with col2:
        spacer(60) # İlk ÇF maçını R1-1 ve R1-2 ortasına it
        m_qf = {}
        for i in range(4):
            m_qf[i] = match_card(f"MQF_{i}", m_r1[i*2][0], m_r1[i*2+1][0], f"Ana ÇF-M{i+1}")
            spacer(185) # ÇF maçları arası boşluk
    
    # YF ve Final
    with col3:
        spacer(220) # YF'yi ÇF'lerin ortasına it
        m_sf = {i: match_card(f"MSF_{i}", m_qf[i*2][0], m_qf[i*2+1][0], f"Ana YF-M{i+1}") for i in range(2)}
        spacer(370) # Finali YF'lerin ortasına it
