...
def render_main_bracket_svg(state):
    ...
    for i, m in enumerate(g["r1"]):
        d = state[m["id"]]
        parts.append(_box_svg(X_R1, m["top"], m["center"], "R1", i+1, d["p1"], d["p2"], d["winner"], d["score"]))
    for j, m in enumerate(g["qf"]):
        d = state[m["id"]]
        parts.append(_box_svg(X_QF, m["top"], m["center"], "ÇF", j+9, d["p1"], d["p2"], d["winner"], d["score"]))
    ...
    for k, m in enumerate(g["sf"]):
        d = state[m["id"]]
        parts.append(_box_svg(X_SF, m["top"], m["center"], "YF", k+13, d["p1"], d["p2"], d["winner"], d["score"]))
        qfa, qfb = g["qf"][2*k], g["qf"][2*k+1]
        parts.append(_connector(X_QF+BOX_W, qfa["center"], X_QF+BOX_W, qfb["center"], m["center"], X_SF))
    d = state["FINAL_MAIN"]
    parts.append(_box_svg(X_F, g["final"]["top"], g["final"]["center"], "FİNAL", 15, d["p1"], d["p2"], d["winner"], d["score"]))
...
def render_consolation_bracket_svg(state):
    ...
    for j, m in enumerate(g["t_r1"]):
        d = state[m["id"]]
        parts.append(_box_svg(X_R1, m["top"], m["center"], "T-R1", j+16, d["p1"], d["p2"], d["winner"], d["score"]))
    for i, m in enumerate(g["t_cf"]):
        d = state[m["id"]]
        parts.append(_box_svg(X_CF, m["top"], m["center"], "T-ÇF", i+20, d["p1"], d["p2"], d["winner"], d["score"]))
        r1 = g["t_r1"][i]
    ...
    for k, m in enumerate(g["t_yf1"]):
        d = state[m["id"]]
        parts.append(_box_svg(X_YF1, m["top"], m["center"], "T-YF1", k+24, d["p1"], d["p2"], d["winner"], d["score"]))
