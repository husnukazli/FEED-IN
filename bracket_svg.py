...
def render_main_bracket_svg(state, cat_name="Erkekler"):
    ...
    for i, m in enumerate(g["r1"]):
        d = state[m["id"]]
        parts.append(_box_svg(X_R1, m["top"], m["center"], "AT-R1", i+1, d["p1"], d["p2"], d["winner"], d["score"], bg_color, line_color, text_color))
    for j, m in enumerate(g["qf"]):
        d = state[m["id"]]
        parts.append(_box_svg(X_QF, m["top"], m["center"], "AT-ÇF", j+9, d["p1"], d["p2"], d["winner"], d["score"], bg_color, line_color, text_color))
    ...
    for k, m in enumerate(g["sf"]):
        d = state[m["id"]]
        parts.append(_box_svg(X_SF, m["top"], m["center"], "AT-YF", k+13, d["p1"], d["p2"], d["winner"], d["score"], bg_color, line_color, text_color))
        qfa, qfb = g["qf"][2*k], g["qf"][2*k+1]
        parts.append(_connector(X_QF+BOX_W, qfa["center"], X_QF+BOX_W, qfb["center"], m["center"], X_SF))
    d = state["FINAL_MAIN"]
    parts.append(_box_svg(X_F, g["final"]["top"], g["final"]["center"], "AT-FİNAL", 15, d["p1"], d["p2"], d["winner"], d["score"], bg_color, line_color, text_color))
...
def render_consolation_bracket_svg(state, cat_name="Erkekler"):
    ...
    for j, m in enumerate(g["t_r1"]):
        d = state[m["id"]]
        parts.append(_box_svg(X_R1, m["top"], m["center"], "FC-R1", j+16, d["p1"], d["p2"], d["winner"], d["score"], bg_color, line_color, text_color))
    for i, m in enumerate(g["t_cf"]):
        d = state[m["id"]]
        parts.append(_box_svg(X_CF, m["top"], m["center"], "FC-ÇF", i+20, d["p1"], d["p2"], d["winner"], d["score"], bg_color, line_color, text_color))
        r1 = g["t_r1"][i]
    ...
    for k, m in enumerate(g["t_yf1"]):
        d = state[m["id"]]
        parts.append(_box_svg(X_YF1, m["top"], m["center"], "FC-YF1", k+24, d["p1"], d["p2"], d["winner"], d["score"], bg_color, line_color, text_color))
    ...
    for k, m in enumerate(g["t_yf2"]):
        d = state[m["id"]]
        parts.append(_box_svg(X_YF2, m["top"], m["center"], "FC-YF2", k+26, d["p1"], d["p2"], d["winner"], d["score"], bg_color, line_color, text_color, p2_kaynak=f"M{k+13} Kaybedeni"))
        yf1 = g["t_yf1"][k]
    ...
    for mid, lbl, no, m in labels:
        d = state[mid]
        parts.append(_box_svg(X_F, m["top"], m["center"], lbl, no, d["p1"], d["p2"], d["winner"], d["score"], bg_color, line_color, text_color))
